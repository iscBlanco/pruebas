...
import { AuthenticationError, AṕolloError, ApolloError } 'apollo-server'
...
export default class AuthAPI extends DataSource{
    ...
    async verifyToken(token){
        ...
        if (!token){
            logger.error('[verilfyToken] missing token')
            throw new AṕolloError ('Missing token', 'MISSING_TOKEN')
        }
